...
def serial_send(key):    
    ...
    try:    
        while True:        # Capture a frame from the camera        
            ...
            print(decision)        # Send the decision to the robot        
            serial_send(decision)        # Display the video feed and processed images        
            cv2.imshow("Frame (Color)", frame_bgr_flipped)        
            cv2.imshow("Grayscale", gray_image)        
            cv2.imshow("Black Image", black_image)        # Exit on 'q' key press        
